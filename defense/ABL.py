...
def custom_loss(y_true, y_pred, gamma):
    ...
    loss = torch.sign(loss - gamma) * loss
    return loss.mean().item()

def find_best_gamma(model, train_loader, val_loader, gamma_values):
    """
    Find the best gamma value using grid search.

    Args:
        model (nn.Module): The model to be trained.
        train_loader (DataLoader): DataLoader for training data.
        val_loader (DataLoader): DataLoader for validation data.
        gamma_values (list): List of gamma values to search.
    ...
    for gamma in gamma_values:
        ...
        with torch.no_grad():
            for data, target in val_loader:
                ...
                output = model(data)
                loss = custom_loss(target, output, gamma)
                total_loss += loss
                count += 1

        avg_loss = total_loss / count
        print(f"Gamma: {gamma}, Avg Loss: {avg_loss}")

        if avg_loss < best_score:
            best_score = avg_loss
            best_gamma = gamma

    print("最佳参数:", best_gamma)
...
class ABL(Base):
    ...
    def train(self, split_ratio, isolation_criterion, gamma, schedule, transform, selection_criterion):
        ...
        # gamma_values = [i * 0.1 for i in range(51)]
        # gamma = find_best_gamma(self.model, self.poisoned_trainset, self.poisoned_testset, gamma_values)
        ...
        if hasattr(self.clean_testset, 'transform'):
            print(self.clean_testset.transform)
            self.poisoned_trainset.transform = self.clean_testset.transform
        if hasattr(self.poisoned_testset, 'poisoned_transform'):
            print(self.poisoned_testset.poisoned_transform)
        ...
        samples = torch.stack([data[0] for data in self.poisoned_trainset])
        labels = torch.LongTensor([data[1] for data in self.poisoned_trainset])
        
        poisoned_samples = samples[poisoned_indices]
        poisoned_labels = labels[poisoned_indices]
        poisoned_dataset = TensorsDataset(poisoned_samples, poisoned_labels, transform)
        other_samples = samples[other_indices]
        other_labels = labels[other_indices]
        other_dataset = TensorsDataset(other_samples, other_labels, transform)
